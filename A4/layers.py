...
class DenseLayer(BaseLayer):
    ...
    def compute_gradient(self):
        ...
        #print(self._output_error_gradient) # 1x1
        #print(self._input_data) # 1 x 2
        #print(np.transpose(np.dot(self._output_error_gradient, self._input_data)))

        self.dw = np.dot(np.transpose(self._input_data), self._output_error_gradient)

        shape = self._output_error_gradient.shape
        #print(shape[0])

        self.db = [np.dot(np.transpose(self._output_error_gradient), np.ones((shape[0],)))]
        #print(self.db)
        #self.db = self.db.reshape(shape_db[0],1)
